/**
 * scripts/translate_phase3_dc.js
 * Tự động dịch và cập nhật toàn bộ 92 Nhân vật DC và 122 Sự kiện DC còn lại.
 */
const fs = require('fs');
...
function refineDcComicText(text) {
  return text
    ...
    .replace(/\bTia chớp\b/gi, 'The Flash')
...
async function processBatch(dataFile, label) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    try {
      ...
      const vi = refineDcComicText(viRaw);

      let html = fs.readFileSync(filePath, 'utf8');

      // 1. Cập nhật Bio paragraph (hỗ trợ cả x-text thông thường và x-text x-content)
...
async function run() {
  await processBatch('scratch/dc_chars_en.json', 'Nhân Vật DC (92 nhân vật)');
  await processBatch('scratch/dc_events_en.json', 'Sự Kiện DC (122 sự kiện)');
  console.log('\n🎉 ĐÃ HOÀN TẤT TOÀN BỘ GIAI ĐOẠN 3 CHO VŨ TRỤ DC!');
}
